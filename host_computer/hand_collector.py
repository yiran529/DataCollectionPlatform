...
class HandCollector:
    ...
    def _init_realtime_write(self) -> bool:
        ...
        try:
            ...
            self._write_queue = queue.Queue(maxsize=50)
...
#             # 创建可变长度数据集（使用chunk优化写入性能）
#             dt = h5py.special_dtype(vlen=np.uint8)
#             # chunk_size=10 与batch_size匹配，减少磁盘碎片
#             self._h5_file.create_dataset('stereo_jpeg', (0,), dtype=dt, maxshape=(None,), chunks=(10,))
#             self._h5_file.create_dataset('mono_jpeg', (0,), dtype=dt, maxshape=(None,), chunks=(10,))
            
#             # 创建固定长度数据集（较大的chunk提升顺序写入性能）
#             self._h5_file.create_dataset('angles', (0,), dtype=np.float32, maxshape=(None,), chunks=(100,))
#             self._h5_file.create_dataset('timestamps', (0,), dtype=np.float64, maxshape=(None,), chunks=(100,))
#             self._h5_file.create_dataset('stereo_timestamps', (0,), dtype=np.float64, maxshape=(None,), chunks=(100,))
#             self._h5_file.create_dataset('mono_timestamps', (0,), dtype=np.float64, maxshape=(None,), chunks=(100,))
#             self._h5_file.create_dataset('encoder_timestamps', (0,), dtype=np.float64, maxshape=(None,), chunks=(100,))
            
#             # 创建写入队列和线程（增大队列避免丢失，队列中每个batch约2-3帧，150可容纳约300-450帧）
#             self._write_queue = queue.Queue(maxsize=150)
#             self._write_thread = threading.Thread(target=self._write_loop, daemon=True)
#             self._write_thread.start()
            
#             print(f"[{self.hand_name}] ✓ 实时写入已启用: {self._h5_filepath}")
#             return True
...
#         encode_params = [cv2.IMWRITE_JPEG_QUALITY, self._jpeg_quality]
#         batch_buffer = []
#         batch_size = 10  # 减小批次加快处理速度，平衡IO次数和响应时间
#         last_write_time = time.time()
#         max_wait_time = 0.15  # 减少等待时间到0.15秒，更快消费队列
...
#                     # 如果单次写入超过0.5秒，打印性能警告
#                     if batch_time > 0.5:
#                         print(f"\n[{self.hand_name}] ⚠️ 写入较慢: {len(batch_buffer)}批耗时{batch_time:.2f}s")
#                     batch_buffer = []
#                     last_write_time = time.time()
...
#         if total_time > 0:
#             avg_throughput = total_batches / total_time
#             print(f"\n[{self.hand_name}] 写入线程完成: {total_batches}批, 耗时{total_time:.1f}s, 平均{avg_throughput:.1f}批/秒")
...
#                     # 刷新到磁盘（每100帧刷新一次，减少IO）
#                     if new_size % 100 == 0 or new_size < 100:
#                         self._h5_file.flush()
