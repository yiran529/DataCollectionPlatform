...
### **最新更新：实时写入模式（2026-01-21）**

#### 📄 host_computer/hand_collector.py - 添加实时写入支持
- **问题**：长时间录制（15秒+）导致OOM（内存耗尽），进程被杀或卡顿
- **解决方案**：新增实时写入模式，录制时直接写入磁盘，避免内存累积
- **改动**：
  - 添加`enable_realtime_write`参数到`__init__`
  - 新增`_init_realtime_write()`：初始化HDF5文件和写入线程
  - 新增`_write_loop()`：独立写入线程，批量处理队列数据
  - 新增`_write_batch()`：批量对齐、压缩、写入数据
  - 修改`_record_loop()`：支持实时写入或内存缓存两种模式
  - 修改`stop_recording()`：实时模式返回文件路径，内存模式返回数据列表
- **内存优化**：从450帧~500MB降至队列缓冲~10-20MB（降低95%）

#### 📄 host_computer/single_hand_collector.py - 适配实时写入
- **改动**：
  - 添加`--realtime-write`命令行参数
  - 传递实时写入配置到`HandCollector`
  - 处理实时模式返回的文件路径（而非数据列表）
- **使用**：`python single_hand_collector.py --record --realtime-write`

#### 📄 host_computer/REALTIME_WRITE_USAGE.md - 使用说明
- **内容**：实时写入模式完整文档
  - 问题背景和解决方案
  - 使用方法和参数配置
  - 性能对比和监控信息
...
## 无法写入
好消息！✅ **数据采集成功了**（对齐后: 2 帧），但数据保存时的 h5py 可变长度数据写入有问题。

需要修复 `save_single_hand_data()` 函数中的 JPEG 数据写入方式： 

读取 [](file:///e%3A/vla/DataCollectionPlatform/host_computer/single_hand_collector.py#95-95)，行 95 到 120

问题在于 h5py 的可变长度数据类型写入。修复方法是将 JPEG 数据转换为字节对象： 
...
这次应该能成功保存数据到 HDF5 文件了！

问题原因：`cv2.imencode()` 返回的是 numpy 数组，但 h5py 的可变长度数据类型 (`vlen=np.uint8`) 需要显式的 bytes 对象或 `np.bytes_` 类型。
